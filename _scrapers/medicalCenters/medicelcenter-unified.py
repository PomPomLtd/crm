#!/usr/bin/env python3
"""
Medical Centers Data Scraper (Unified Version)
Scrapes swiss medical centers from onedoc.ch using unified base scraper class.
"""
...
class MedicalCentersScraper(BaseHealthcareScraper):
    """Medical Centers data scraper using unified base class"""
    
    def __init__(self):
        super().__init__('medical-centers')
    ...
    def extract_item_details(self, url: str, item_name: str, index: int, total: int) -> Dict[str, Any]:
        """Extract detailed information from swiss medical centers from onedoc.ch page"""
        details = {
            'phone_number': "",
            'professions': [],
            'email': "",
...
def main():
    """Entry point"""
    scraper = MedicalCentersScraper()
    try:
        result = scraper.run()
        print(f"Scraping completed. Processed {result} swiss medical centers from onedoc.ch.")
