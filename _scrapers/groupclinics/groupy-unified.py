#!/usr/bin/env python3
"""
Group Practices Data Scraper (Unified Version)
Scrapes swiss group practices from onedoc.ch using unified base scraper class.
"""
...
class GroupPracticesScraper(BaseHealthcareScraper):
    """Group Practices data scraper using unified base class"""
    
    def __init__(self):
        super().__init__('group-practices')
    ...
    def extract_item_details(self, url: str, item_name: str, index: int, total: int) -> Dict[str, Any]:
        """Extract detailed information from swiss group practices from onedoc.ch page"""
        details = {
            'phone_number': "",
            'professions': [],
            'email': "",
...
def main():
    """Entry point"""
    scraper = GroupPracticesScraper()
    try:
        result = scraper.run()
        print(f"Scraping completed. Processed {result} swiss group practices from onedoc.ch.")
