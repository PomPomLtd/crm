...
# Base URL for the hospitals overview page
overview_url = "https://www.onedoc.ch/de/spital"  # URL for hospitals
...
def extract_hospital_details(url, hospital_name, hospital_index, total_hospitals):
    """Extract additional details (phone number and professions) from a hospital detail page"""
    details = {
    ...
    print(f"[{hospital_index}/{total_hospitals}] Getting details for: {hospital_name}")
    
    # Fetch the detail page
    html = fetch_page(url)
    if not html:
        print(f"  ❌ Failed to get details for {hospital_name}")
        return details
...
def extract_hospitals(html, canton_name):
    """Extract hospital data from a canton page"""
    if not html:
        return []
    
    soup = BeautifulSoup(html, 'html.parser')
    hospital_items = soup.find_all('div', class_='directory-item')
    
    total_items = len(hospital_items)
    print(f"Found {total_items} hospitals on this page")
    
    hospitals = []
    for index, item in enumerate(hospital_items, 1):
        # Extract hospital name and URL
        name_element = item.find('a')
        name = name_element.text.strip() if name_element else "Unknown"
        hospital_url = "https://www.onedoc.ch" + name_element['href'] if name_element and name_element.has_attr('href') else "Unknown"
        ...
        # Fetch additional details from the hospital detail page if URL is valid
        if hospital_url != "Unknown":
            details = extract_hospital_details(hospital_url, name, index, total_items)
            hospital_data['phone_number'] = details['phone_number']
            hospital_data['professions'] = ", ".join(details['professions'])
        ...
        # Show progress after each hospital
        print(f"Progress: {index}/{total_items} hospitals processed on this page")
...
def check_processed_pages():
    ...
    processed = {}
    if os.path.exists('hospital_processed_pages.csv'):
        with open('hospital_processed_pages.csv', 'r', encoding='utf-8') as f:
            reader = csv.reader(f)
            for row in reader:
                if len(row) >= 2:
                    canton = row[0]
...
def mark_page_processed(canton, page_num):
    ...
    with open('hospital_processed_pages.csv', 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([canton, page_num])

def load_already_scraped_hospitals():
    """Load already scraped hospitals to avoid duplicates"""
    scraped_urls = set()
    if os.path.exists('hospitals_progress.csv'):
        with open('hospitals_progress.csv', 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                scraped_urls.add(row.get('url', ''))
    return scraped_urls

def extract_hospitals_with_progress(html, canton_name, scraped_urls):
    """Extract hospital data from a canton page with progress display"""
    hospitals = extract_hospitals(html, canton_name)
    
    # Filter out already scraped hospitals
    new_hospitals = []
    for hospital in hospitals:
        if hospital['url'] not in scraped_urls:
            new_hospitals.append(hospital)
            scraped_urls.add(hospital['url'])
    
    return new_hospitals

def save_to_csv(data, filename="all_hospitals.csv"):
    """Save the hospitals data to a CSV file"""
    if not data:
    ...
    print(f"Saved {len(data)} hospitals to {filename}")
    print(f"File saved at: {os.path.abspath(filename)}")

def save_progress(data, filename="hospitals_progress.csv"):
    ...
    print(f"Saved progress: {len(data)} hospitals appended to {filename}")
...
def main():
    ...
    scraped_urls = load_already_scraped_hospitals()
    print(f"Found {len(scraped_urls)} already scraped hospitals")
    ...
    for canton_index, canton in enumerate(canton_links):
        ...
        if not first_page_processed:
            ...
            hospitals_page1 = extract_hospitals_with_progress(canton_html, canton_name, scraped_urls)
            print(f"Found {len(hospitals_page1)} new hospitals on page 1")
            
            all_hospitals.extend(hospitals_page1)
            total_count += len(hospitals_page1)
            
            # Save progress after each page
            save_progress(hospitals_page1)
            mark_page_processed(canton_name, 1)
            
            # Display overall progress
            print(f"Overall progress: {total_count} hospitals scraped so far")
        else:
        ...
        for page_num in range(2, max_page + 1):
            ...
            hospitals = extract_hospitals_with_progress(page_html, canton_name, scraped_urls)
            print(f"Found {len(hospitals)} new hospitals on page {page_num}")
            
            all_hospitals.extend(hospitals)
            total_count += len(hospitals)
            
            # Save progress after each page
            save_progress(hospitals)
    ...
    save_to_csv(all_hospitals)
    print(f"Scraping completed. Total new hospitals found: {len(all_hospitals)}")
    print(f"Total hospitals overall: {total_count}")
