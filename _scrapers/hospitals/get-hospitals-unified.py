#!/usr/bin/env python3
"""
Hospitals Data Scraper (Unified Version)
Scrapes swiss hospitals from onedoc.ch using unified base scraper class.
"""
...
class HospitalsScraper(BaseHealthcareScraper):
    """Hospitals data scraper using unified base class"""
    ...
    def extract_item_details(self, url: str, item_name: str, index: int, total: int) -> Dict[str, Any]:
        """Extract detailed information from swiss hospitals from onedoc.ch page"""
        details = {
            'phone_number': "",
            'professions': [],
            'email': "",
...
def main():
    """Entry point"""
    scraper = HospitalsScraper()
    try:
        result = scraper.run()
        print(f"Scraping completed. Processed {result} swiss hospitals from onedoc.ch.")
