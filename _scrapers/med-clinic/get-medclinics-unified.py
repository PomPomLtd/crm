#!/usr/bin/env python3
"""
Medical Clinics Data Scraper (Unified Version)
Scrapes swiss medical clinics from onedoc.ch using unified base scraper class.
"""
...
class MedicalClinicsScraper(BaseHealthcareScraper):
    """Medical Clinics data scraper using unified base class"""
    
    def __init__(self):
        super().__init__('medical-clinics')
    ...
    def extract_item_details(self, url: str, item_name: str, index: int, total: int) -> Dict[str, Any]:
        """Extract detailed information from swiss medical clinics from onedoc.ch page"""
        details = {
            'phone_number': "",
            'professions': [],
            'email': "",
...
def main():
    """Entry point"""
    scraper = MedicalClinicsScraper()
    try:
        result = scraper.run()
        print(f"Scraping completed. Processed {result} swiss medical clinics from onedoc.ch.")
