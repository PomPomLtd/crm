#!/usr/bin/env python3
"""
Complete Directory Data Scraper (Unified Version)
Scrapes complete healthcare directory from onedoc.ch using unified base scraper class.
"""
...
class CompleteDirectoryScraper(BaseHealthcareScraper):
    """Complete Directory data scraper using unified base class"""
    
    def __init__(self):
        super().__init__('complete-directory')
    ...
    def extract_item_details(self, url: str, item_name: str, index: int, total: int) -> Dict[str, Any]:
        """Extract detailed information from complete healthcare directory from onedoc.ch page"""
        details = {
            'phone_number': "",
            'professions': [],
            'email': "",
...
def main():
    """Entry point"""
    scraper = CompleteDirectoryScraper()
    try:
        result = scraper.run()
        print(f"Scraping completed. Processed {result} complete healthcare directory from onedoc.ch.")
